...
export default function PreviewAboutTreatmentImage({
	...
	const imageMarginTop =
		imageContainerRef.current && imageHeight > imageContainerHeight
	...
	return (
		<div className={styles.imageInputZone}>
			<div className={styles.imagePreview}>
				{error && (
					<p className={`error ${styles.errorMessage}`}>{error.message}</p>
				)}
				...
				<p className={styles.caption}>Попередній перегляд</p>
			</div>
		</div>
	);
}
