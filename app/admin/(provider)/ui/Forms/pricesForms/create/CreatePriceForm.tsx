...
export default function CreatePriceForm() {
	...
	useEffect(() => {
		if (optionalServiceRef.current) {
			dispatch(
				setOptionalServiceInputHeight(
					optionalServiceRef.current.scrollHeight,
				),
			);
		}
	});
	...
	useEffect(() => {
		if (priceVariantsWrapRef.current) {
			dispatch(
				setPriceVariantsHeight(priceVariantsWrapRef.current.scrollHeight),
			);
		}
	});
	...
	const createPriceSection: SubmitHandler<PriceSectionFormData> = async (data) => {
		...
		const response = await dispatch(
			createPriceSectionAction({ data, departmentId }),
		);
	...
	const handleOptionalServiceCheckbox = (
	...
	const handlePriceCountOptionCheckbox = (
		e: React.ChangeEvent<HTMLInputElement>,
	) => {
		const state = e.target.checked;
		dispatch(triggerMeetingsCountCheckbox(state));
	};
	const handlePriceDurationOptionCheckbox = (
		e: React.ChangeEvent<HTMLInputElement>,
	) => {
		const state = e.target.checked;
		dispatch(triggerMeetingDurationCheckbox(state));
	};
	const handlePriceMeetingPriceOptionCheckbox = (
		e: React.ChangeEvent<HTMLInputElement>,
	) => {
		const state = e.target.checked;
		dispatch(triggerMeetingPriceCheckbox(state));
	};
	const handlePriceTotalPriceOptionCheckbox = (
		e: React.ChangeEvent<HTMLInputElement>,
	) => {
		const state = e.target.checked;
	...
	return (
		<form onSubmit={handleSubmit(createPriceSection)} className={styles.form}>
			<AnimatePresence>
			...
			<div className={styles.optionalService}>
				<div
					className={`${styles.top} ${optionalServiceCheckbox ? styles.active : ""}`}
				>
					<label className={`inputLabel`} htmlFor="optionalService">
							...
							>
								<input
									...
									{...register(PriceSectionEnum.OPTIONALSERVICE, {
										required: optionalServiceCheckbox
											? "Якщо вибрали, додаткову послугу, то введіть її назву"
											: false,
									})}
			...
			>
				...
				<div className={styles.checkboxes}>
					...
					<div className={styles.checkbox}>
						<label className={`inputLabel`} htmlFor="durationCheckbox">
							Тривалість заняття
						</label>
						<Checkbox
					...
					<div className={styles.checkbox}>
						<label className={`inputLabel`} htmlFor="priceCheckbox">
							Ціна за 1 заняття
						</label>
						<Checkbox
					...
					<div className={styles.checkbox}>
						<label className={`inputLabel`} htmlFor="totalPriceCheckbox">
							Ціна за весь курс
						</label>
						<Checkbox
			...
			<div className={styles.priceVariantsWrap}>
				<div className={styles.top}>
					<label className={`title md`} htmlFor="priceVariants">
						Варіації послуги
					</label>
			...
			<div className={styles.formErrorWrap}>
				{error.create && (
					<p className={`error ${styles.formError}`}>
