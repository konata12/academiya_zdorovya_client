...
export default function PriceFormTitle({
	...
	const handlePriceNearTitleCheckbox = (
		...
		>
			...
			>
				...
				<div className={styles.title}>
					...
					>
						{index ? "Назва послуги" : "Повна назва послуги"}
					</label>
					<input
						className={`input`}
						type="text"
						...
						{...register(`titles.${index}.${PriceTitleEnum.TEXT}`, {
							required:
								"Якщо вибрали, додатку послугу, то введіть її назву",
						})}
					/>
				...
				<div className={styles.priceNearTitle}>
					...
					<input
						...
						type="text"
						{...register(
							`titles.${index}.${PriceTitleEnum.PRICENEARTITLE}`,
							{
								required: titleWithPrice
									? "Введіть ціну біля назви або відключіть її"
									: false,
							},
						)}
					/>
