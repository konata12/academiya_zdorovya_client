...
export default function SingleServiceData() {
	const service = useAppSelector((state: RootState) => state.serviceCreateForm);
	const imageUrl = useGetImageUrlFromIndexedDBImage(service.image, "service_create_images");
					...
					<div className={styles.cardsContainer}>
						{service.serviceTypes.map((data, i) => (
							<ServiceTypeCardPreview
								serviceType={data}
								imageStoreName={"service_create_images"}
								key={i}
							/>
						))}
					</div>
