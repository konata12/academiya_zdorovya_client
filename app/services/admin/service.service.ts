		...
		for (const key in data) {
			...
			if (key === ServiceFormDataEnum.IMAGE) {
				if (typeof value !== "string")
					throw Error("Помилка BACKGROUNDIMG при створенні послуги");
				...
				typedValues.forEach((typedValue, i) => {
					if (
						typedValue[ServiceTreatmentStageEnum.TITLE] &&
						typedValue[ServiceTreatmentStageEnum.DESCRIPTION]
					) {
						formData.append(`${key}[${i}][order]`, `${i}`);
						formData.append(
							`${key}[${i}][${ServiceTreatmentStageEnum.TITLE}]`,
							typedValue[ServiceTreatmentStageEnum.TITLE],
						);
						formData.append(
							`${key}[${i}][${ServiceTreatmentStageEnum.DESCRIPTION}]`,
							typedValue[ServiceTreatmentStageEnum.DESCRIPTION],
						);
					}
				});
				...
				employees.forEach((employee, i) => {
					formData.append(`${key}[${i}][order]`, `${i}`);
					formData.append(
						`${key}[${i}][${ServiceEmployeesFormDataEnum.ID}]`,
						`${employee[ServiceEmployeesFormDataEnum.ID]}`,
					);
				});
...
export const updateServiceFormData = async (data: CreateServiceFormData) => {
	try {
		...
		for (const key in data) {
			...
			if (key === ServiceFormDataEnum.IMAGE) {
				if (typeof value !== "string")
					throw Error("Помилка IMAGE при створенні послуги");
...
async function parseServiceTypesResponse(
	...
	const parsedServiceTypes = await Promise.all(
	...
	return parsedServiceTypes.sort((a, b) => {
		return a.order - b.order;
	});
}

// PARSE RESPONSE DATA TO FORM DATA
export function parseServiceToServiceFormData(service: Service): ServiceFormData {
	const { id, serviceTypes, employees, ...data } = service;
...
export async function transferServiceImagesBetweenIndexDBStores(
	...
	try {
		...
		if (service.serviceTypes) {
			await Promise.all(
				service.serviceTypes.map(async (type) => {
					await transferImageBetweenIndexDBStores(
						type.backgroundImg,
						getStoreName,
						setStoreName,
					...
					await transferDetailsRedactorTypeImagesBetweenIndexDBStores(
						type.details,
						getStoreName,
						setStoreName,
					);
