import {
	...
	BookingServiceInit,
} from "@/app/types/data/booking_services.type";
...
export const createBookingService = createAsyncThunk(
	...
	async (data: BookingServiceFormData, { rejectWithValue }) => {
		try {
			const response = await axiosInstance.post<BookingService[]>(
				`${requesUrlBase}/admin/create`,
				data,
			);
			console.log(response);
			return response.data;
...
const bookingServicesSlice = createSlice({
	...
	extraReducers(builder) {
		builder
			...
			.addCase(
				fetchBookingServices.fulfilled,
				(state, action: PayloadAction<BookingService[] | undefined>) => {
			...
			.addCase(deleteBookingService.rejected, (state, action) => {
				...
				if (
					...
					const index = state.bookingServices.findIndex(
						(bookingServices) => bookingServices.id === error.id,
					);
					if (index !== -1) {
...
export const { openModalBookingServices, closeModalBookingServices } =
