...
const employeesFormUISlice = createSlice({
	...
	reducers: {
		...
		setEmployeeUIFormValues(
			...
			const { workSpecialities, achivements } = action.payload;
			...
			for (const key in action.payload) {
				switch (key) {
					...
					case EmployeesFormDataEnum.ACHIVEMENTS:
						...
						state[`${key}Checkbox`] =
							valueAch && !!valueAch.length ? true : false;
						break;
				}
			}

			// SET MODAL VALUES
			state.workSpecialitysModalIsOpen = new Array(
				workSpecialities.length,
			).fill(false);
			if (achivements)
				state.achivementsModalIsOpen = new Array(achivements.length).fill(
					false,
				);
		},
	},
