...
export type DraggableComponentsDataType =
	| OrderedListServiceEmployeeInterface
	| OrderedListServiceTypeInterface;
export type DraggableComponentsType =
	| ServiceEmployeeFormData
	| ServiceTypeServiceFormData;
...
export function useOrderedList() {
	...
	const handleDragEnd = useCallback(
		(event: DragEndEvent, data: DraggableComponentsDataType) => {
			const { order, valueName, sliceName } = data;
			...
			const activeItemIndex = order.findIndex(
				(item) => item.orderId === active.id,
			);
			const overItemIndex = order.findIndex(
				...
				case "servicesCreateForm":
					switch (valueName) {
						case ServiceFormDataEnum.SERVICETYPES:
							dispatch(
								setServiceCreateTypes(
									arrayMove(order, activeItemIndex, overItemIndex),
								),
							);
							break;
