...
export function useServiceTypeFormCheckChange(newServiceType: Omit<ServiceTypeFormData, 'errors' | 'orderId'>) {
    const {
        errors,
        ...data
    } = useAppSelector((state: RootState) => state.serviceUpdateForm)
    const {services} = useAppSelector((state: RootState) => state.services)
    const {id, serviceTypeIndex} = useParams<{ id: string, serviceTypeIndex: string }>()
