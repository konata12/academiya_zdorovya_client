...
export function useEmployeeFormHandleChange(
    ...
    const handleChange = useCallback(<T extends FormElements>({
        ...
        if (socialMediaFields.includes(elementType as EmployeeSocialMediaKeysType)) {
            // REQUIRED ERROR HANDLING 
            if (newValue.length > 0) dispatch(setBasicValueError({
            ...
            dispatch(setSocialMediaValue({
                field: elementType as EmployeeSocialMediaKeysType,
                value: newValue
            }))
        }
        if (stringArrayFields.includes(elementType as EmployeeStringArrayKeysType)) {
            // REQUIRED ERROR HANDLING
            if (arrIndex !== undefined) {
                if (newValue.length > 0) dispatch(setStringArrayValueError({
                    field: elementType as EmployeeStringArrayKeysType,
                    index: arrIndex,
                    message: ''
                }))

                dispatch(setStringArrayValue({
                    field: elementType as EmployeeStringArrayKeysType,
                    index: arrIndex,
                    value: newValue
                }))
