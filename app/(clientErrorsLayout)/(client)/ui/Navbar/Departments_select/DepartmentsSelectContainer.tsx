...
import {
	...
	DepartmentsResponse,
} from "@/app/(clientErrorsLayout)/(client)/types";
import React, { useEffect, useRef, useState } from "react";
import styles from "./DepartmentsSelectContainer.module.scss";
import { DepartmentSelect } from "@/app/(clientErrorsLayout)/(client)/ui/Navbar/Departments_select/DepartmentSelect/DepartmentSelect";
...
export default function DepartmentsSelectContainer() {
	...
	async function fetchData() {
		try {
			...
			document.cookie = `departmentId=${result[0].id}; path=/;`;
		} catch (err: unknown) {
